...
public class YearDataService
{
    ...
    /// </summary>
    public IEventData? GetYearData(int year) => 
    ...
    /// 모든 행사 데이터 조회 (최신순)
    /// </summary>
    public IEnumerable<IEventData> GetAllYearData() => _allEvents;

    /// <summary>
    /// 가장 최근 행사 데이터 조회
    /// </summary>
    public IEventData? GetLatestYearData() => _allEvents.FirstOrDefault();
    ...
    /// 이전 연도 데이터 조회 (하위 호환성)
    /// </summary>
    public IEventData? GetPreviousYearData(int year)
    {
        var index = _availableYears.IndexOf(year);
        if (index < 0 || index >= _availableYears.Count - 1)
            return null;
        return GetYearData(_availableYears[index + 1]);
    }

    /// <summary>
    /// 다음 연도 데이터 조회 (하위 호환성)
    /// </summary>
    public IEventData? GetNextYearData(int year)
    {
        var index = _availableYears.IndexOf(year);
        if (index <= 0)
            return null;
    ...
    /// 연도별 타임라인 정보 생성
    /// </summary>
    public IEnumerable<YearTimelineItem> GetTimeline()
    {
        return _availableYears.Select(year =>
        {
            var data = GetYearData(year)!;
            return new YearTimelineItem
            {
                Year = year,
                Slug = data.Slug,
...
/// </summary>
public record YearTimelineItem
{
